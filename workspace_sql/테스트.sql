...
-- 문제2
-- job이 CLERK 이거나 급여 2000초과 이면서 부서 번호 10인 사원만 출력
select * from emp 
...
-- 컬럼이 같고 or 연결되어 있는 경우
-- in으로 간편하게 표현 가능
select * from emp
where depino in (20,30);
...
select * from emp
where lower(ename) like upper('%Am%');
...
from emp 
where	
	(deptno = 20 or deptno = 30) and ename like '%A%';
...
-- 왼쪽부터 적용되고 동일한 값이 있는 경우 다음 조건이 적용이 된다
select * from emp 
...
-- 문제3 
-- 부서 번호가 20 또는 30인 사원중에서 
-- sal 2000~3000 사이(포함)인 사원의
-- sal이 작은 순으로 출력하세요.
